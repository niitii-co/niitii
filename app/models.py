...
from app.enums import AccountPermission, UserViewer, PostViewer, FlagReason
...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    def can(self, key, utc = None):
        if self.permission == AccountPermission.ROOT_ADMIN.value:
            return True
        if utc and (utc - 1) <= self.utc_offset <= (utc + 1):
...
# invalidate token by setting expiration date to one second before current time
    ...
    def _del_comments(self):
        db.session.execute(sa.delete(Comment).where(Comment.user_id == self.id))

    def _del_flags(self):
        db.session.execute(sa.delete(Flag).where(Flag.user_id == self.id))

    def _del_votes(self):
        db.session.execute(sa.delete(Vote).where(Vote.user_id == self.id))

    def _del_notifications(self):
        db.session.execute(sa.delete(Notification).where(Notification.user_id == self.id))
...
# check if token is invalid or expired
    ...
    @staticmethod
    def check_user_agent(token, time, name):
# human validation
        if token and time and name == "":
            pass
        else:
            return False
...
# time validation
        if timedelta(seconds=3) < time_elapsed < timedelta(seconds=600):
            return True
...
class Post(db.Model):
    ...
    def get_img_tags(self):
        parser = CustomParser('img')
        parser.feed(self.body_html)
        imgs = parser.found_tags
        return imgs
    ...
    def del_tags(self, tag_ls=None):
        for tag in self.tags:
            if tag_ls and tag.name in tag_ls:
                pass
            else:
                tag.del_self()

    def _del_comments(self):
        db.session.execute(sa.delete(Comment).where(Comment.post_id == self.id))

    def _del_flags(self):
        db.session.execute(sa.delete(Flag).where(Flag.post_id == self.id))

    def _del_votes(self):
        db.session.execute(sa.delete(Vote).where(Vote.post_id == self.id))
