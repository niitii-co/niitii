...
follows = sa.Table(
    'follows',
    db.metadata,
    sa.Column('follower_id', sa.BigInteger, sa.ForeignKey('user.id', ondelete='CASCADE'), primary_key=True),
    sa.Column('followed_id', sa.BigInteger, sa.ForeignKey('user.id', ondelete='CASCADE'), primary_key=True),
...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    __tablename__ = 'user'
    id: so.Mapped[int] = so.mapped_column(sa.BigInteger, primary_key=True, default=SystemRandom().randrange(1000000000, 9999999999, 1))
...
#   generate token valid for one hour
    ...
    def update_notification_count(self):
        ...
        if notice:
            ...
            notice.update_payload({"count": f"{count}"})
...
tags = sa.Table(
    'tags',
    db.metadata,
    sa.Column('post_id', sa.BigInteger, sa.ForeignKey('post.id', ondelete='CASCADE'), primary_key=True),
    sa.Column('tag_id', sa.BigInteger, sa.ForeignKey('tag.id', ondelete='CASCADE'), primary_key=True),
    sa.Column('timestamp', sa.DateTime, default=lambda: datetime.now(timezone.utc))
)
...
class Post(db.Model):
    ...
    user_id: so.Mapped[int] = so.mapped_column(sa.BigInteger, sa.ForeignKey('user.id', ondelete='CASCADE'))
    ...
    def pin_comments_count(self):
        query= sa.select(Comment).where((Comment.post_id == self.id) & (Comment.pin_comments == True))
        count = db.session.execute(sa.select(sa.func.count()).select_from(query.subquery())).scalar()
        self.direct_comments = count
        return count
...
class Vote(db.Model):
    __talename__ = 'vote'
    id: so.Mapped[int] = so.mapped_column(sa.BigInteger, primary_key=True)
    user_id: so.Mapped[int] = so.mapped_column(sa.BigInteger, sa.ForeignKey('user.id', ondelete='CASCADE'), index=True)
...
class Message(db.Model):
    ...
    id: so.Mapped[int] = so.mapped_column(sa.BigInteger, primary_key=True)
    sender_id: so.Mapped[int] = so.mapped_column(sa.BigInteger, sa.ForeignKey('user.id', ondelete='CASCADE'))
    sender: so.Mapped['User'] = so.relationship(foreign_keys='Message.sender_id', back_populates='messages_sent')
    recipient_id: so.Mapped[int] = so.mapped_column(sa.BigInteger, sa.ForeignKey('user.id', ondelete='CASCADE'))
