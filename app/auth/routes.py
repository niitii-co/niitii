...
def login():
    ...
    if request.method == 'POST':
        if not User.check_user_agent(request.form.get('csrf_token'), request.form.get('startTime'), request.form.get('name')):
        ...
        if user and user.mfa_enabled:
            if not request.form.get('mfa_token') or not user.check_totp(int(request.form.get('mfaToken'))) and user.admin_token != int(request.form.get('mfaToken')):
        ...
        user.utc_offset = request.form.get('utcOffset') if request.form.get('utcOffset') else 0
        db.session.commit()
...
def setup_mfa():
    if request.method == 'POST':
        if current_user.check_totp(request.form.get('mfaToken')):
...
def change_password():
    if request.method == 'POST':
        if current_user.check_password(request.form.get('oldPassword')) and request.form.get('password') == request.form.get('password2'):
