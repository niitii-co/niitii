...
from app.models import User, Post, Comment, Conversation, Message, Notification, Tag, Vote, Permission, Photo, Flag, FlagReason, Chatii
...
def comment(id):
    ...
    if not post.disable_comments:
        ...
        utc_offset = request.form.get('utc_offset')
        comment = Comment(author=current_user._get_current_object(), body=request.form.get('body'), post_id=id, user_id=current_user.id, pinned=pinned, language=g.language, utc_offset=utc_offset)
...
# PRAGMA enables cascade in sqlite
        db.session.execute(sa.text('PRAGMA foreign_keys = ON'))
        user.del_all_sent_messages() #db.session.execute(user.messages_sent.delete())
        user.del_all_comments() #db.session.execute(user.comments.delete())
        user.del_all_votes() #db.session.execute(user.votes.delete())
        user.del_all_flags #db.session.execute(user.flags.delete())
        user.del_all_notifications() #db.session.execute(user.notifications.delete())
        user.del_all_posts()
