...
class Post(db.Model):
    ...
    def get_index(lang):
        if current_user.is_authenticated:
            return sa.select(Post).where((Post.language == current_user.language) & (Post.utc_offset.between(current_user.utc_offset - 1, current_user.utc_offset + 1))).order_by(Post.votes.desc(), Post.comments.desc(), Post.timestamp.desc()).limit(200)
        else:
            return sa.select(Post).where(Post.language == lang).order_by(Post.votes.desc(), Post.comments.desc(), Post.timestamp.desc()).limit(200)
