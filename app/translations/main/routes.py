...
# PRAGMA enables cascade in sqlite
        db.session.execute(sa.text('PRAGMA foreign_keys = ON'))
        user.del_sent_messages()
        user.del_comments()
        user.del_votes()
        user.del_flags()
        user.del_notifications()
        user.del_posts()
        user.del_follows()
        user.del_user()
...
# Create a new mailbox with highest mailbox + 1
    ...
    return {
        ...
        "photo": Photo.url(message.photos()['link'].removeprefix(f"{Photo.SPACES_URL}/message-pics/")) if message.photos() else None
    }
...
def del_message(id):
    ...
    if message.photos:
        bucket = 'message-pics'
        name = message.photos()['link'].removeprefix(f"{Photo.SPACES_URL}/{bucket}/")
...
#@login_required
#def export_posts():
#    if current_user.task_in_progress('export_posts'):
