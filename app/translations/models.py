...
#            'last_seen': self.last_seen.replace(tzinfo=timezone.utc).isoformat(),
            ...
            '_links': {
                'self': url_for('api.user', id=self.id),
                'followers': url_for('api.followers', id=self.id),
                'following': url_for('api.following', id=self.id),
...
#    Store as (BLOB or BYTEA) to work with Fernet encrypt-decrypt
    ...
    def url(self):
        ...
        return Photo.url(photos.removeprefix(f"{Photo.SPACES_URL}/message-pics/"))
