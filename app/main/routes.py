...
def comment(id):
    ...
    if not post.disable_comments:
        ...
        db.session.flush()
        post.author.update_notification_count()
...
def reply_comment(id):
    ...
    if not post.disable_comments:
        ...
        if comment:
            ...
            db.session.flush()
            c_author.update_notification_count()
...
def del_comment(id):
    ...
    if (current_user != comment.author and (current_user == post.author or current_user.can('MODERATE'))):
        if comment.pinned:
            post.pin_comments -= 1
...
def view_notification():
    ...
    current_user.update_notification_count()
...
# query mailbox and message_id. message_id used in convo loop.
    ...
    current_user.update_message_count()
...
@login_required
def invite_chat():
    user = User.query_user(request.form.get('body').lower(), True)
